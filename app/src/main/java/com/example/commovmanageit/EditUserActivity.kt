...
import com.example.commovmanageit.EditProfileActivity
...
class EditUserActivity : AppCompatActivity()  {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            user = app.userRepository.getByIdRemote(userId)?.toLocal()
            ...
            if (userRole.isNotEmpty()) {
                bottomNav?.let {
                    BottomNavigationHelper(this@EditUserActivity, it, userRole).setup()
                }
            }
        }
        ...
        btnSave.setOnClickListener {
            ...
            lifecycleScope.launch {
                val user = userRepository.getByIdRemote(intent.getStringExtra("userId") ?: "")
                    ?.toLocal()
                val exists = userRepository.getByemailRemote(email)
                if (exists != null && exists.email != user?.email) {
                    Toast.makeText(this@EditUserActivity, getString(R.string.email_already_registered), Toast.LENGTH_SHORT).show()
                    return@launch
                }
                ...
                user?.let {
                    ...
                    Toast.makeText(this@EditUserActivity, getString(R.string.profile_updated), Toast.LENGTH_SHORT).show()
                    finish()
                }
            }
        }
